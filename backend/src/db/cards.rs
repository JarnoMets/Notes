...
impl Database {
    pub async fn get_cards(&self, list_id: &str) -> DbResult<Vec<Card>> {
        let rows = sqlx::query(
            r#"SELECT id, list_id, title, description, position, due_date, labels, archived, created_at, updated_at 
    ...
    pub async fn get_archived_cards(&self, board_id: &str) -> DbResult<Vec<Card>> {
        let rows = sqlx::query(
            r#"SELECT c.id, c.list_id, c.title, c.description, c.position, c.due_date, c.labels, c.archived, c.created_at, c.updated_at 
               FROM cards c
    ...
    pub async fn create_card(&self, card: &Card) -> DbResult<Card> {
        ...
        sqlx::query(
            "INSERT INTO cards (id, list_id, title, description, position, due_date, labels, archived, created_at, updated_at) 
    ...
    pub async fn update_card(
        ...
        sqlx::query(
            "UPDATE cards SET title = $1, description = $2, position = $3, due_date = $4, labels = $5, archived = $6, list_id = $7, updated_at = $8 WHERE id = $9"
        )
        .bind(&new_title)
